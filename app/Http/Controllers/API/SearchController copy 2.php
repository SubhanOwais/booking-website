...
class SearchController extends Controller
{
    ...
    /**
     * Resolve the company-specific city ID from the CompanyCity table.
     *
     * Looks up: company_id == $operatorId AND key_id == $globalCityId
     * Returns the company's own city_id, or falls back to $globalCityId
    ...
    private function resolveCompanyCityId(int $operatorId, int $globalCityId): ?int
    {
        try {
            ...
            // No mapping found — signal caller to skip this operator
    ...
    public function search(Request $request)
    {
        ...
        return response()->stream(function () use ($companies, $fromId, $toId, $date) {
            ...
            try {
                ...
                $anyDiscount = null;
                try {
                    $anyDiscount = Discount::forCity($toId)->first();
                } catch (Exception $e) {
                    Log::warning('Failed to fetch discount', ['toId' => $toId, 'error' => $e->getMessage()]);
                }
                ...
                foreach ($companies as $index => $company) {
                    ...
                    // ── Resolve company-specific city IDs ─────────────────────
                    // Each company may use its own internal city IDs that differ
                    // from our global city IDs. We look up the mapping here so
                    // the correct IDs are sent to that company's API.
                    ...
                    try {
                        // ── Schedule fetch ────────────────────────────────────
                        // Uses $companyFromId / $companyToId instead of raw fromId/toId
                        $scheduleResponse = Http::timeout(1)
                        ...
                        // ── Parallel fare requests ────────────────────────────
                        // Also uses $companyFromId / $companyToId for the fare API
                        $farePoolRequests = [];
                        ...
                        // ── Discount for this company ─────────────────────────
                        $discount = null;
                        try {
                            $discount = Discount::forCity($toId)->forCompany((int) $operatorId)->first();
                        } catch (Exception $e) {
                            Log::warning("Discount fetch failed for {$companyName}", ['error' => $e->getMessage()]);
                        }
                        ...
                        foreach ($grouped as $serviceTypeId => $serviceGroup) {
                            try {
                                ...
                                $builtTrip = [
                                    ...
                                    'fromCityId'          => $fromId,        // Global ID kept for frontend
                                    'toCityId'            => $toId,          // Global ID kept for frontend
                                    'companyFromCityId'   => $companyFromId, // Mapped ID for reference
                                    'companyToCityId'     => $companyToId,   // Mapped ID for reference
                        ...
                        $this->sseEmit('company_results', [
                            ...
                            'discount'    => $anyDiscount
                                ? ['percentage' => $anyDiscount->discount_percentage, 'active' => true]
                                : null,
                        ]);
    ...
    private function applyDiscountToTrip(array $trip, float $pct): array
    {
        try {
            ...
            if (isset($trip['seat_4_fare']))  $trip['seat_4_fare']  = round($trip['seat_4_fare']  - ($trip['seat_4_fare']  * $pct / 100), 2);
            if (isset($trip['seat_20_fare'])) $trip['seat_20_fare'] = round($trip['seat_20_fare'] - ($trip['seat_20_fare'] * $pct / 100), 2);
    ...
    private function calculateDuration(string $departure, string $arrival): string
    {
        try {
            ...
            Log::warning('calculateDuration failed', [
                'departure' => $departure,
                'arrival'   => $arrival,
                'error'     => $e->getMessage(),
            ]);
