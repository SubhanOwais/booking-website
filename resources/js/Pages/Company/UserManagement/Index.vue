                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-200">
                                <th
                                    class="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide w-12">
                                    #</th>
                                <th
                            ...
                            <tr v-for="(user, index) in filteredUsers" :key="user.id"
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="relative flex-shrink-0">
                                            <img v-if="user.profile_picture" :src="'/storage/' + user.profile_picture"
                                        ...
                                        <div>
                                            <p class="font-medium text-slate-800 text-sm leading-tight">{{ user.name }}
                                            </p>
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        ...
                                        :class="user.is_active ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'">
                                        <span class="w-1.5 h-1.5 rounded-full"
                                            :class="user.is_active ? 'bg-green-500' : 'bg-red-400'"></span>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <button v-if="!isCompanyOwner(user)" @click="openEditModal(user)"
                                        ...
                                        <button v-if="!isCompanyOwner(user)" @click="openDeleteModal(user.id)"
            ...
            <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col z-10">
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                        <div>
                            <h2 class="text-base font-bold text-slate-800">{{ editID ? 'Edit User' : 'Create New User'
                            }}</h2>
                            <p class="text-xs text-slate-400 mt-0.5">{{ editID ? 'Update user information and roles' :
                                'Fill in the details to add a new user' }}</p>
                        </div>
                    ...
                    <div class="overflow-y-auto flex-1 px-6 py-4 space-y-4">
                        ...
                        <div>
                            <label
                                class="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Profile
                                Picture</label>
                            <div class="border border-slate-200 rounded-xl bg-gray-100 p-3 overflow-y-auto max-h-[239px]"
                                :class="{ 'border-red-400': errors.userImage }">
                                <div class="avatar-grid">
                                    <div v-for="(avatar, idx) in defaultAvatars" :key="idx"
                                        ...
                                        <img :src="'/images/avatars/' + avatar" class="w-full h-full object-cover"
                                            :alt="'Avatar ' + (idx + 1)" />
                                    </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 mb-1.5">Full Name <span
                                        class="text-red-400">*</span></label>
                                <input type="text" v-model="form.name" @input="clearError('name')"
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 mb-1.5">Email Address <span
                                        class="text-red-400">*</span></label>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-slate-600 mb-1.5">Phone Number <span
                                        class="text-red-400">*</span></label>
                                <input type="tel" v-model="form.phone_number" @input="clearError('phone_number')"
                                    placeholder="+92 300 0000000"
                                    class="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                    :class="errors.phone_number ? 'border-red-400 bg-red-50' : 'border-slate-200'" />
                                <p v-if="errors.phone_number" class="text-xs text-red-500 mt-1">{{ errors.phone_number
                                }}</p>
                            </div>
                            ...
                            <div v-if="!editID || (editID && isLoggedInUserOwner)">
                                ...
                                <div class="relative">
                                    <input :type="showPassword ? 'text' : 'password'" v-model="form.password"
                                        @input="clearError('password')" placeholder="••••••••"
                            ...
                            <div class="relative" ref="roleDropdownRef">
                                ...
                                <div class="relative">
                                    <button type="button" @click="toggleRoleDropdown"
                                        ...
                                        <span class="text-sm text-slate-700">{{ selectedRolesText || 'Select role(s)'
                                        }}</span>
                                    ...
                                    <div v-if="roleDropdownOpen"
                                        ...
                                        <div v-for="role in roleList" :key="role.value"
                                            class="px-3 py-2 hover:bg-slate-50 flex items-center gap-2">
                                            <input type="checkbox" :value="role.value" v-model="form.role"
                                                :id="'role-' + role.value"
                                                class="w-4 h-4 text-primary rounded border-slate-300 focus:ring-primary" />
                                            <label :for="'role-' + role.value"
                                                class="text-sm text-slate-700 cursor-pointer flex-1">{{ role.label
                                                }}</label>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <div
                                    class="flex items-center justify-between px-3 py-2 border border-slate-200 rounded-lg">
                                    <span class="text-sm text-slate-600">{{ form.is_active ? 'Active' : 'Inactive'
                                    }}</span>
                                    <button type="button" @click="form.is_active = !form.is_active"
                                        class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none"
                                        :class="form.is_active ? 'bg-green-700' : 'bg-red-700'">
                                        <span
                    ...
                    <div
                        ...
                        <button @click="closeModal"
                            class="px-4 py-2 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">Cancel</button>
                    ...
                    <div class="p-6">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-500 mt-1 leading-relaxed">Are you sure you want to delete
                                    this user? This action is permanent and cannot be undone.</p>
                            </div>
                        </div>
                    </div>
...
const fetchUsers = async () => {
    try {
        ...
        toast.info('Loaded', 'Users fetched successfully.')
    } catch (error) {
        console.error(error)
...
const fetchRoles = async () => {
    try {
        ...
        roleList.value = Array.isArray(data)
            ? data.map(r => ({ label: r.name, value: r.name }))
            : []
    } catch (error) {
        console.error(error)
...
const submitForm = async () => {
    ...
    try {
        ...
        // ✅ TOAST FIX
        toast.success(
            editID.value ? 'User Updated' : 'User Created',
