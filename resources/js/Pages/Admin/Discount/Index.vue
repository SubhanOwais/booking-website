...
const fetchMappedCities = async (mainCityId) => {
    ...
    try {
        const response = await axios.get(`/admin/discount/mapped-cities/${mainCityId}`);
...
const submitCreate = () => {
    createForm.post(route('admin.discount.store'), {
        onSuccess: () => {
            closeCreateModal();
        }
    });
...
const submitEdit = () => {
    editForm.put(route('admin.discount.update', selectedDiscount.value.id), {
        onSuccess: () => {
            closeEditModal();
        }
    });
...
const confirmDelete = () => {
    router.delete(route('admin.discount.destroy', selectedDiscount.value.id), {
        onSuccess: () => {
            closeDeleteModal();
        }
    });
...
const toggleActive = (id) => {
    router.post(route('admin.discount.toggle-active', id));
};

const restoreDiscount = (id) => {
    router.post(route('admin.discount.restore', id));
};
...
const forceDeleteDiscount = (id) => {
    if (confirm('Are you sure you want to permanently delete this discount? This action cannot be undone!')) {
        router.delete(route('admin.discount.force-delete', id));
    }
};
            ...
            <div class="p-4 mb-6 bg-white rounded-lg shadow">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input v-model="searchQuery" type="text" placeholder="Search by name, cities, or companies..."
                            class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-700">Status Filter</label>
        ...
        <teleport to="body">
            <div v-if="showCreateModal" class="fixed inset-0 z-50 overflow-y-auto">
                <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                    <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" @click="closeCreateModal">
                    </div>
                    <div
                                    ...
                                    <div class="w-full">
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div v-if="createForm.main_city_id">
                                                <label class="block mb-2 text-sm font-medium text-gray-700">Select
                                                    Mapped Cities
                                                    * (Hold Ctrl/Cmd for multiple)</label>
                                                <div v-if="loadingMappedCities" class="text-sm text-gray-500">Loading
                                                    cities...
                                                </div>
                                                <select v-else v-model="createForm.mapped_city_ids" multiple required
                                                    class="block w-full h-40 mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                    <option v-for="city in mappedCities" :key="city.id"
                                                        :value="city.id">{{
                                                            city.name }}</option>
                                                </select>
                                                <div class="mt-1 text-xs text-gray-500">Selected: {{
                                                    createForm.mapped_city_ids.length }} cities</div>
                                                <div v-if="createForm.errors.mapped_city_ids"
                                                    class="mt-1 text-sm text-red-600">
                                                    {{ createForm.errors.mapped_city_ids }}</div>
                                            </div>

                                            <div>
                                                <label class="block mb-2 text-sm font-medium text-gray-700">
                                                    Select Companies * (Hold Ctrl/Cmd for multiple)
                                                </label>
                                                <select v-model="createForm.company_ids" multiple required
                                                    class="block w-full h-32 mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                    <option v-for="company in availableCompanies" :key="company.id"
                                                        :value="company.id">
                                                        {{ company.name }}
                                                    </option>
                                                </select>
                                    ...
                                    <div class="w-full">
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div v-if="editForm.main_city_id">
                                                ...
                                                <select v-else v-model="editForm.mapped_city_ids" multiple required
                                                    class="block w-full h-40 mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                    <option v-for="city in mappedCities" :key="city.id"
                                                        :value="city.id">{{
                                                            city.name }}</option>
                                                ...
                                                <div class="mt-1 text-xs text-gray-500">Selected: {{
                                                    editForm.mapped_city_ids.length }} cities</div>
                                                <div v-if="editForm.errors.mapped_city_ids"
                                                    class="mt-1 text-sm text-red-600">{{
                                                        editForm.errors.mapped_city_ids }}</div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <select v-model="editForm.company_ids" multiple required
                                                    class="block w-full h-32 mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                    <option v-for="company in availableCompanies" :key="company.id"
                                                        :value="company.id">
                                                        {{ company.name }}
                                                ...
                                                <div v-if="editForm.errors.company_names"
                                                    class="mt-1 text-sm text-red-600">
                                                    {{ editForm.errors.company_names }}
                                                </div>
                                            </div>
